  ...
  <div>
    ...
    <div class="container">
      <v-card width="600" class="mx-auto">
        <v-card-title class="text-h4 justify-center mt-5">
          Join ONE Now
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text>
          <v-form ref="regForm" v-model="valid" class="pa-3">
            ...
            <v-row class="pl-8" dense>
              <v-col cols="12" md="2">
                <v-select v-model="selectedTitle" :items="titles" :rules="regFormRules.title" label="Title"></v-select>
              </v-col>
              <v-col cols="12" md="5">
                <v-text-field v-model="regForm.firstName" :rules="regFormRules.name" label="First name"></v-text-field>
              </v-col>
              <v-col cols="12" md="5">
                <v-text-field v-model="regForm.lastName" :rules="regFormRules.name" label="Last name*"></v-text-field>
              </v-col>
            </v-row>
            <div class="d-flex align-center">
              <v-checkbox class="d-inline-block" v-model="serviceAgreed"></v-checkbox>
              <span
                >By clicking the box, you accept the <a @click="serviceShow = true">Terms and Conditions</a> and
                acknowledge the Privacy Policy</span
              >
            </div>
            <div class="g-recaptcha" data-sitekey="6Lfe1sAZAAAAAK9JZU1zNJHwP-CKbde5tNCC2AUh"></div>
            <v-card-actions>
              <v-btn height="50px" :disabled="isFormValid" block color="success" @click="handleRegister"
    ...
    <v-dialog v-model="serviceShow" scrollable persistent max-width="500px" transition="dialog-transition">
      <v-card>
        <v-card-title>
        ...
        <v-card-text
          ...
          integer, purus sed lorem pulvinar habitasse turpis. +</v-card-text
        >
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn
  ...
  methods: {
    handleRegister() {
      this.$store.dispatch('register', this.regForm).then((res) => {
        if (res.status === '1') {
          this.snackbar = true;
          this.snackText = 'Registration successful!';
          setTimeout(() => {
    ...
    handleGoogle() {
      ...
      let data = {
        secret: '6Lfe1sAZAAAAAO40D4SEzP-CGB1EVXrRQw2GE-18',
        response: response,
      };
      console.log('handling google shit', response);
